...
Zeta potentials were measured for a veraity of particles and solution combination.
The Result of which can be found in Table\ref{Table:zeta_pot}.

\subsection{BSA protocol}
...
4. The third time supernatant is removed, the \acp{MP} are resuspended in \SI{50}{\micro\L} BSA solution of \SI{0.1}{\m\g\per(\m\L)} concentration and \SI{20}{\micro\L} \ac{HEPES}.
The suspension was incubated overnight with \SI{10}{\s} vortexing for three times in the first three hours.
5. The suspension was then washed three times as described above and finally resuspended in \SI{100}{\micro\L} \ac{HEPES} with \SI{0.1}{\percent} polysorbate 20, commercially known as TWEEN 20, which is a non-ionic surfactant. 
This is now the stock suspension of \acp{MP} with \ac{BSA}.
6. For use, \SI{1}{\micro\L} stock suspension is added to \SI{200}{\micro\L} \ac{HEPES} buffer, giving a final bead concentration of $c=\SI{2e6}{beads\per(\milli\L)}$ 