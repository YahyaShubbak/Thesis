...
In general, the evaluation consists of feature location and subsequent temporal linking.
...
Then, local maxima are identified as particle centers using the brightness of the particles, and the brightness and size of each particle are measured\cite{Crocker1996}.
A good starting point for feature finding for images of bright, well-resolved spherical particles of several micrometers in diameter against a dark, static background, using a 20$\times$ objective and a 1920$\times$1080 resolution camera, is:  
\begin{verbatim} 
    diameter    :   13
    minmass     : 2000
    separation  :   12
...
\paragraph{Filtering steps}\label{paragraph:trackpy_filtering}
...
It contains 2959 unique trajectories. 
...
The algorithm then loses track of one or both MPs, causing fragmented trajectories.
...
Fig. \ref{fig:Traj_2000filter} shows a trajectory map after such a filtering step, keeping  417 trajectories.
Here, trajectories that are shorter than \SI{2000}{\frames}, which represent \SI{40}{\percent} of the total duration, are removed.
...
However, it was found that the theoretical fit of the overall mean velocity is represented better when filtering more heavily. 
This could have been avoided if a smaller density of MPs had been chosen for the experiment, and this will be discussed later.
\begin{figure}
    \centering
...
Alongside the fragmentation, which is caused by the linking algorithm of \verb|trackpy|, Fig. \ref{fig:Traj_2000filter} shows a physical artifact as well.
...
In the linear regime, however (Equation \ref{eq:velocity_regimes}), MPs do not perform any oscillatory motion around a DW and do follow the changing potential energy landscape.
Trajectories should therefore have the length in accordance with Equation \ref{eq:traj_length} with the given duration of \SI{5}{\s} at \SI{3.01}{Hz} and a period length $d=\SI{10}{\micro\m}$ be \SI{150.5}{\micro\m}.
This, of course, is only true for MPs who are present for the whole duration, e.g., in an initial left-to-right transport (positive $x-$direction), MPs start near $x=0$ and move to higher $x$ before the phase relation change flips the transport direction, and the MPs are transported back.
Shorter trajectories are possible if MPs leave the ROI by starting at a higher $x$, or enter it at a given time.
However, short trajectories that present themselves as circles are, at least in the linear regime, actually pinned and therefore falsely lower the mean velocity of the ensemble in the linear regime.
...
This leaves the map with 341 trajectories.
Filtering for MPs $<\SI{10}{\micro\m}$ tends to keep some of the oscillating MPs, if the total oscillation duration lasts for more than \SI{10}{\micro\m}, which is the reason \SI{20}{\micro\m} is chosen.
\begin{figure}
    \centering
...
Table \ref{Table:filtering_summary} shows a small summary of the filtering steps and the resulting number of trajectories kept. 

\begin{table}[htb]
\caption{Summary of filtering steps and resulting number of trajectories and mean velocity for \ch{COOH} MPs at \SI{3.01}{\Hz} and \SI{10}{\Hz} driving frequency, representing linear and non-linear regimes respectively.}
\label{Table:filtering_summary}
\begin{tabular}{lllll}
Experiment    & Regime     & Trajectories removed                       & n Trajectories    & $\left<\left<v\right>\right>$(expected) / $\si{\micro\m\per\s}$ \\
\SI{3.01}{\Hz}& linear     & No filtering                               & 2959              & \SI{34.37}{\micro\m\per\s} (\SI{30.10}{\micro\m\per\s})\\
              &            & $<\SI{2000}{\frames}$                      & 417               & \SI{24.55}{\micro\m\per\s} (\SI{30.10}{\micro\m\per\s})\\
              &            & $<\SI{20}{\micro\m}$                       & 495               & \SI{30.11}{\micro\m\per\s} (\SI{30.10}{\micro\m\per\s})\\
              &            & $<\SI{2000}{\frames}$ + $<\SI{20}{\micro\m}$ & 341             & \SI{30.05}{\micro\m\per\s} (\SI{30.10}{\micro\m\per\s})\\
\SI{10}{\Hz}  & non-linear & No filtering                               & 4626              & \SI{43.28}{\micro\m\per\s} (\SI{23.75}{\micro\m\per\s})\\
              &            & $<\SI{2000}{\frames}$                      & 380               & \SI{24.84}{\micro\m\per\s} (\SI{23.75}{\micro\m\per\s})                       
\end{tabular}
\end{table}