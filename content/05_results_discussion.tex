...
Such a graph is presented in Figure \ref{fig:x-t-graph_100ms} for the linear regime.
It shows a single \ch{COOH}-micromer \ac{MP} that was transported and tracked the entire duration of the video, that is, $t=\SI{5}{s}$.
...
Note that the ramp time shown is exaggerated for visual purposes.
A full period $\mathrm{T}$ is shown containing the aforementioned small step starting at $t=\SI{2200}{ms}$, and the large step starting at $t=\SI{2300}{ms}$.
The force equilibrium is reached within nanoseconds (refer to theory or cite Holzinger?) which is not resolvable at the \si{ms} time scale the camera can provide.
