...
The red rectangle is shown as a zoom in in Figure \ref{fig:x-t-graph_100ms_zoom} to more clearly present the individual steps and how they are correlated with the alternating pulsed field sequence, depicted on top.
\begin{figure}[tbh]
    \centering
...
Therefore, the acceleration phase is assumed to be instantaneous.
why not linear but gradual stop after field application?
After the instantaneous acceleration, the particle moves linearly and is then decelerated before it comes to a stop at the adjacent \ac{DW}.
$\mathrm{T} = \SI{400}{ms}$ can be nicely observed, since $\nicefrac{T}{4}=\SI{100}{ms}$. 

\begin{figure}[tbh]
    \centering
    \includegraphics[width=0.8\linewidth]{Abbildungen/Evaluation/x_t_COOH_100ms_zoom.pdf}
    \caption{
    $x$ over $t$ graph of a transport experiment in the linear regime.
    }
